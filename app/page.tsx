...
export default function App() {
    ...
    reader.onload = () => {
      try {
        ...
        publicadores.forEach((p) => {
          const tieneRegistrosPrecursor = p.months.some(
            (m) => m.pio.toLowerCase() === "reg"
          );
        ...
        const publicadoresOrdenados = publicadores.sort((a, b) => {
          ...
          const aAuxiliar =
            a.groupname.toLowerCase().includes("auxiliar") ||
            a.groupname.toLowerCase().includes("auxiliary");
          const bAuxiliar =
            b.groupname.toLowerCase().includes("auxiliar") ||
            b.groupname.toLowerCase().includes("auxiliary");
        ...
        if (publicadoresOrdenados.length > 0) {
          ...
          setSelectedPub(0);
          alert(
            `✅ Archivo cargado exitosamente: ${publicadoresOrdenados.length} publicadores encontrados`
          );
        } else {
  ...
  const parseXML = (
    text: string
  ): { publicadores: Publicador[]; metadata: MetaData } => {
    try {
      ...
      const publicadores: Publicador[] = Array.from(pubElements).map((pub) => {
          ...
          Array.from(monthEls).forEach((monthEl) => {
            ...
            if (year) {
              months.push({
                ...
                year,
                plcmts:
                  monthEl.getElementsByTagName("Plcmts")[0]?.textContent || "0",
                videos:
                  monthEl.getElementsByTagName("Videos")[0]?.textContent || "0",
                hours:
                  monthEl.getElementsByTagName("Hours")[0]?.textContent || "0",
                rvs:
                  monthEl.getElementsByTagName("R.V.s")[0]?.textContent || "0",
                bist:
                  monthEl.getElementsByTagName("BiSt.")[0]?.textContent || "0",
                remark:
                  monthEl.getElementsByTagName("Remark")[0]?.textContent || "",
        ...
        return {
          ...
          groupname:
            pub.getElementsByTagName("groupname")[0]?.textContent || "",
          status:
            pub.getElementsByTagName("status")[0]?.textContent || "activo",
  ...
  const obtenerEtiquetas = (pub: Publicador) => {
    ...
    if (
      statusLower.includes("expulsado") ||
      statusLower.includes("expelled") ||
      statusLower.includes("sacado")
    ) {
      etiquetas.push({ texto: "Sacado", color: "bg-gray-700" });
    } else if (
    ...
    if (
      groupnameLower.includes("auxiliar") ||
      groupnameLower.includes("auxiliary")
    ) {
      etiquetas.push({ texto: "Auxiliar", color: "bg-orange-500" });
    }

    const esPrecursorCampo =
      pioneerLower.includes("regular") || pioneerLower === "reg";
    ...
    if (
      groupnameLower.includes("anciano") ||
      groupnameLower.includes("elder")
    ) {
      etiquetas.push({ texto: "Anciano", color: "bg-red-500" });
    }

    if (
      groupnameLower.includes("ministerial") ||
      groupnameLower.includes("servant")
    ) {
      ...
      pubs
        .filter((p) => {
          ...
          return (
            ...
            !status.includes("expulsado") &&
            !status.includes("expelled") &&
            !status.includes("sacado")
          );
        })
        .map((p) => p.group)
    )
  ...
  const descargarCaptura = () => {
    ...
    canvas.width = 1920;
    const publicadorHeight = 145;
    const headerHeight = 120;
    const padding = 50;
    const columnWidth = (canvas.width - padding * 3) / 2;
    ...
    ctx.fillText(
      `${
        pubsParaCaptura.length
      } publicadores • ${new Date().toLocaleDateString()}`,
      padding,
      100
    );
    ...
    pubsParaCaptura.forEach((p, index) => {
      ...
      const isAuxiliar =
        p.groupname.toLowerCase().includes("auxiliar") ||
        p.groupname.toLowerCase().includes("auxiliary");
      const statusLower = (p.status || "activo").toLowerCase();
      const isInactivo =
        statusLower.includes("inactivo") || statusLower.includes("inactive");
      const isSacado =
        statusLower.includes("expulsado") ||
        statusLower.includes("expelled") ||
        statusLower.includes("sacado");
      ...
      let etiquetasFiltradas = etiquetas.filter(
        (e) => e.texto !== "Super" && e.texto !== "Auxiliar"
      );

      if (selectedGroup !== "inactivos") {
        etiquetasFiltradas = etiquetasFiltradas.filter(
          (e) => e.texto !== "Sacado" && e.texto !== "Inactivo"
        );
      }
      ...
      if (selectedGroup === "inactivos") {
        ctx.fillText(
          `Grupo ${p.group} • ID: ${p.id} • ${generoES}`,
          xPos + 15,
          textY
        );
      } else {
      ...
      const direccion =
        p.addr && p.addr.length > maxDirLength
  ...
  const editarCampo = (campo: keyof Publicador, valor: string) => {
    ...
    setPubs((prev) => {
      ...
      if (campo === "group" && valor !== grupoAnterior) {
        ...
        const nuevoIndice = copiaOrdenada.findIndex(
          (p) => p.id === publicadorActual.id
        );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-2 sm:p-6">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg p-3 sm:p-6 mb-3 sm:mb-6">
          <h1 className="text-2xl sm:text-4xl font-bold text-gray-800 mb-3 sm:mb-4">
            📋 Editor de Publicadores XML
          </h1>
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-3 sm:mb-4">
            <label className="px-4 sm:px-6 py-2 sm:py-3 bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-700 transition shadow-md font-semibold text-sm sm:text-base text-center">
              📁 Cargar XML
              <input
                type="file"
                accept=".xml"
            ...
            {pubs.length > 0 && (
              <>
                <button
                  ...
                  className="px-4 sm:px-6 py-2 sm:py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition shadow-md font-semibold text-sm sm:text-base"
                >
                ...
                {selectedGroup && (
                  <button
                    ...
                    className="px-4 sm:px-6 py-2 sm:py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition shadow-md font-semibold text-sm sm:text-base"
                  >
          ...
          {meta.agent && (
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-xs sm:text-sm text-gray-600 bg-gray-50 p-3 sm:p-4 rounded-lg">
              <span>
                <strong>Agente:</strong> {meta.agent} v{meta.agentVer}
              </span>
              <span>
                <strong>Fecha:</strong> {meta.date}
              </span>
              <span>
                <strong>Total:</strong> {pubs.length} publicadores
              </span>
            </div>
          )}{" "}
        </div>
        ...
        {pubs.length === 0 ? (
          <div className="bg-white rounded-xl shadow-lg p-8 sm:p-12 text-center">
            ...
            <p className="text-lg sm:text-xl text-gray-600">
              Carga un archivo XML para comenzar
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-3 sm:gap-6">
            {/* SELECTOR DE GRUPOS */}
            <div className="lg:col-span-1 bg-white rounded-xl shadow-lg p-3 sm:p-4 max-h-[300px] lg:max-h-[calc(100vh-250px)] overflow-y-auto">
              <h2 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 text-gray-700 sticky top-0 bg-white z-10">
                Grupos
              </h2>
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        const primerPubDelGrupo = pubs.findIndex(
                          (p) => p.group === grupo
                        );
            ...
            <div className="lg:col-span-1 bg-white rounded-xl shadow-lg p-3 sm:p-4 max-h-[300px] lg:max-h-[calc(100vh-250px)] overflow-y-auto">
              <h2 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 text-gray-700 sticky top-0 bg-white pb-2 z-10">
                {selectedGroup === "inactivos"
                  ? "🚫 Inactivos"
                  : `Grupo ${selectedGroup}`}
              </h2>
              {pubsDelGrupo.map((p, i) => {
                ...
                const isSuperGrupo = p.groupname
                  .toLowerCase()
                  .includes("super");
                const isAuxiliar =
                  p.groupname.toLowerCase().includes("auxiliar") ||
            ...
            <div className="lg:col-span-2 bg-white rounded-xl shadow-lg p-3 sm:p-6 max-h-[600px] lg:max-h-[calc(100vh-250px)] overflow-y-auto">
              {pub && (
                <>
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {obtenerEtiquetas(pub).map((etiq, idx) => (
                          <span
                            key={idx}
                            className={`text-xs sm:text-sm px-2 sm:px-3 py-1 rounded-full text-white ${etiq.color}`}
                          >
                            {etiq.texto}
                          </span>
                        ))}
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto px-3 sm:px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition shadow-md font-semibold text-sm sm:text-base whitespace-nowrap"
                    >
                      🗑️ Eliminar
                    </button>
                  </div>
                        ...
                        <div key={field}>
                          ...
                          {type === "select" && field === "gender" ? (
                            <select
                              value={pub.gender}
                              onChange={(e) =>
                                editarCampo("gender", e.target.value)
                              }
                    ...
                    <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-300">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-3 sm:mb-4">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={pub.groupname
                              .toLowerCase()
                              .includes("super")}
                            onChange={(e) => {
                              if (e.target.checked) {
                                if (
                                  !pub.groupname.toLowerCase().includes("super")
                                ) {
                                  editarCampo(
                                    "groupname",
                                    pub.groupname
                          ...
                          <input
                            ...
                            checked={
                              pub.groupname
                                .toLowerCase()
                                .includes("auxiliar") ||
                              pub.groupname.toLowerCase().includes("auxiliary")
                            }
                            onChange={(e) => {
                              if (e.target.checked) {
                                if (
                                ...
                                editarCampo(
                                  ...
                                  pub.groupname
                                    .replace(
                                      /auxiliar(\s*de\s*grupo)?|auxiliary/gi,
                                      ""
                                    )
                                    .trim()
                                );
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={
                              pub.groupname.toLowerCase().includes("anciano") ||
                              pub.groupname.toLowerCase().includes("elder")
                            }
                            onChange={(e) => {
                              if (e.target.checked) {
                                if (
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            checked={
                              ...
                              pub.groupname.toLowerCase().includes("servant")
                            }
                            onChange={(e) => {
                              if (e.target.checked) {
                                if (
                                ...
                                editarCampo(
                                  ...
                                  pub.groupname
                                    .replace(
                                      /ministerial|siervo\s*ministerial|servant/gi,
                                      ""
                                    )
                                    .trim()
                                );
                      ...
                      <div className="pt-2 sm:pt-3 border-t border-gray-200">
                        ...
                        <p className="text-xs text-red-600 mt-2">
                          ⚠️ Los inactivos y sacados no aparecen en los grupos,
                          solo en la pestaña "Inactivos"
                        </p>
                      </div>
